...
function PenaltyGame({ onGol, onSkip }) {
  ...
  const patearTarget=(tx,ty)=>{
    ...
    setBalPos({x:tx,y:ty});
    setTimeout(()=>{
      // Puntos dorados = siempre gol, el portero no puede atraparlo
      setFase('gol'); setTimeout(()=>onGol(),600);
    },560);
    // Portero intenta moverse al punto pero llega tarde
    setPorteroX(px => {
      const diff = tx - px;
      return Math.max(22, Math.min(78, px + diff * 0.3));
    });
  };
...
const MEXICO_CSS = `
  ...
  .dash-header {
    background: rgba(0,61,42,0.97) !important;
  ...
  .bottom-nav {
    ...
    box-shadow: 0 -8px 40px rgba(0,40,25,0.5) !important;
  }
  .bottom-nav-item.active {
    background: rgba(206,17,38,0.15) !important;
  }
  .bottom-nav-item.active .bottom-nav-icon {
    filter: drop-shadow(0 4px 8px rgba(206,17,38,0.5)) !important;
  }
