    ...
    <aside className={`sidebar${collapsed ? ' collapsed' : ''}`}>
      ...
      <nav className="sidebar-nav">
        {itemsFiltrados.map(item => (
          <div key={item.id} className={`nav-item${seccion === item.id ? ' active' : ''}`}
      ...
      <div className="sidebar-footer">
        ...
        <div className="sidebar-user">
          {usuario?.foto_url
            ...
            : <div style={{ width: 38, height: 38, borderRadius: '50%', background: 'rgba(255,255,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 20, border: '2px solid var(--dorado)' }}>👤</div>
          }
        ...
        <button className="btn-institucional"
          style={{ width: '100%', borderColor: 'rgba(255,255,255,0.4)', color: 'rgba(255,255,255,0.85)', fontSize: 13 }}
          onClick={logout}>
          🚪 Cerrar Sesión
        </button>
      </div>
