...
export default function OrganigramaPage() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'#F7F8FC',fontFamily:'Montserrat,sans-serif'}}>
      {/* Header */}
      <div style={{background:'linear-gradient(135deg,#1a1a2e,#2d2d4e,#C9A84C)',padding:'20px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:12}}>
        <div>
          <div style={{fontSize:12,color:'rgba(255,255,255,0.5)',fontWeight:700,letterSpacing:2,textTransform:'uppercase'}}>SITT · Estructura</div>
          <div style={{fontSize:28,fontWeight:900,color:'#fff',fontFamily:'Playfair Display,serif',fontStyle:'italic'}}>Organigrama</div>
        </div>
      ...
      <div style={{maxWidth:1200,margin:'0 auto',padding:'32px 16px'}}>
                ...
                {sinJerarquia.map(e=>{
                  ...
                  return (
                    <div key={e.id} onClick={()=>setSel(e)}
                      style={{background:'#fff',borderRadius:14,padding:'12px 16px',textAlign:'center',border:`2px dashed ${color.border}`,cursor:'pointer',width:148,boxShadow:'0 2px 8px rgba(0,0,0,0.06)',transition:'all 0.2s',opacity:0.8}}
          ...
          <div className="modal-overlay" onClick={()=>setSel(null)}>
            <div style={{background:'#fff',borderRadius:20,maxWidth:380,width:'95%',overflow:'hidden',boxShadow:'0 20px 60px rgba(0,0,0,0.3)'}} onClick={e=>e.stopPropagation()}>
              <div style={{background:'linear-gradient(135deg,#6B0F2B,#9B1540)',padding:'24px',textAlign:'center',position:'relative'}}>
                <button onClick={()=>setSel(null)} style={{position:'absolute',top:12,right:12,background:'rgba(255,255,255,0.15)',border:'none',color:'#fff',width:28,height:28,borderRadius:'50%',cursor:'pointer',fontSize:16}}>✕</button>
                {sel.foto_url
                  ? <img src={sel.foto_url} alt="" style={{width:90,height:90,borderRadius:'50%',objectFit:'cover',border:'4px solid #C9A84C',marginBottom:12}}/>
                  : <div style={{width:90,height:90,borderRadius:'50%',background:'rgba(255,255,255,0.15)',display:'flex',alignItems:'center',justifyContent:'center',fontSize:32,color:'#C9A84C',margin:'0 auto 12px',fontWeight:900}}>{sel.nombre?.[0]}{sel.apellido_paterno?.[0]}</div>
                }
                <div style={{fontSize:20,fontWeight:900,color:'#fff',fontFamily:'Playfair Display,serif',fontStyle:'italic'}}>{sel.nombre} {sel.apellido_paterno}</div>
                <div style={{fontSize:13,color:'#C9A84C',fontWeight:700,marginTop:4}}>{sel.cargo_funcional||sel.puesto||'—'}</div>
              </div>
              <div style={{padding:'20px 24px',display:'flex',flexDirection:'column',gap:10}}>
                {[['🏢 Departamento',sel.departamento],['📧 Correo',sel.email]].map(([l,v])=>(
        ...
        {/* Panel jerarquía modal */}
        {panelId && esAdmin && (() => {
          const empPanel = empleados.find(e=>e.id===panelId);
          if (!empPanel) return null;
          const color = getDeptColor(empPanel.departamento);
          return (
            <div className="modal-overlay" onClick={()=>setPanelId(null)}>
              <PanelJerarquia emp={empPanel} empleados={empleados} color={color}
    ...
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',position:'relative'}}>
      ...
      <div style={{position:'relative', paddingTop:14}}>
        <div onClick={()=>{ if(!editandoCargo) onClickEmp(node); }}
          ...
          {node.departamento && <div style={{position:'absolute',top:-18,left:'50%',transform:'translateX(-50%)',background:color.bg,color:color.text,fontSize:7,fontWeight:900,padding:'2px 10px',borderRadius:20,letterSpacing:1,whiteSpace:'nowrap',zIndex:10,boxShadow:`0 2px 8px ${color.bg}60`}}>{node.departamento.toUpperCase()}</div>}
          ...
          {editandoCargo&&esAdmin ? (
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:4,marginTop:3}}>
              ...
              {esAdmin && <button onClick={e=>{e.stopPropagation();setEditandoCargo(true);}} style={{background:'none',border:'none',cursor:'pointer',fontSize:9,opacity:0.4,padding:0}}>✏️</button>}
            </div>
          )}

          {esAdmin && (
            <button onClick={e=>{e.stopPropagation();setPanelId(panelId===node.id?null:node.id);}}
              style={{marginTop:8,padding:'5px 12px',borderRadius:20,border:`1px solid ${color.border}`,background:panelId===node.id?color.bg:'transparent',color:panelId===node.id?color.text:color.bg,fontSize:10,fontWeight:800,cursor:'pointer',fontFamily:'Montserrat,sans-serif',width:'100%',transition:'all 0.2s'}}>
              ⚙️ Jerarquía {panelId===node.id?'▲':'▼'}
            </button>
          )}
        </div>
      </div>
...
function PanelJerarquia({ emp, empleados, color, onActualizar, onCerrar }) {
  ...
  return (
    <div onClick={e=>e.stopPropagation()} style={{background:'#fff',borderRadius:20,padding:'24px',boxShadow:'0 20px 60px rgba(0,0,0,0.3)',width:'min(320px,92vw)',border:`2px solid ${color.border}`}}>
      <div style={{fontWeight:800,fontSize:13,color:color.bg,marginBottom:16,textAlign:'center'}}>⚙️ {emp.nombre} {emp.apellido_paterno}</div>
      ...
      <select value={jefeId} onChange={e=>setJefeId(e.target.value)} style={{width:'100%',padding:'8px 10px',borderRadius:9,border:`1.5px solid ${color.border}`,fontFamily:'Montserrat,sans-serif',fontSize:12,marginBottom:12,boxSizing:'border-box'}}>
      ...
      <select value={depto} onChange={e=>setDepto(e.target.value)} style={{width:'100%',padding:'8px 10px',borderRadius:9,border:`1.5px solid ${color.border}`,fontFamily:'Montserrat,sans-serif',fontSize:12,marginBottom:depto==='__nuevo__'?6:12,boxSizing:'border-box'}}>
      ...
      {depto==='__nuevo__' && (
        <input placeholder="Nombre del departamento" value={deptoNuevo} onChange={e=>setDeptoNuevo(e.target.value)}
          style={{width:'100%',padding:'8px 10px',borderRadius:9,border:`1.5px solid ${color.border}`,fontFamily:'Montserrat,sans-serif',fontSize:12,marginBottom:12,boxSizing:'border-box'}}/>
      )}
      ...
      <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',marginBottom:12}}>
        ...
        <span style={{fontSize:12,fontWeight:700,color:'#4A5568'}}>Es asistente</span>
      </label>
      ...
      <input type="number" min="0" max="99" value={nivel} onChange={e=>setNivel(e.target.value)}
        style={{width:'100%',padding:'8px 10px',borderRadius:9,border:`1.5px solid ${color.border}`,fontFamily:'Montserrat,sans-serif',fontSize:12,marginBottom:16,boxSizing:'border-box'}}/>

      <div style={{display:'flex',gap:8}}>
        <button onClick={onCerrar} style={{flex:1,padding:'10px',borderRadius:9,border:`1px solid ${color.border}`,background:'#fff',cursor:'pointer',fontFamily:'Montserrat,sans-serif',fontWeight:700,fontSize:12,color:'#718096'}}>Cancelar</button>
        <button onClick={guardar} style={{flex:2,padding:'10px',borderRadius:9,border:'none',background:color.bg,color:color.text,cursor:'pointer',fontFamily:'Montserrat,sans-serif',fontWeight:800,fontSize:12}}>✓ Guardar</button>
      </div>
    </div>
  );
}
