  ...
  return (
    <div style={{fontFamily:'Montserrat,sans-serif'}}>
      ...
      <div style={{textAlign:'center',marginBottom:32}}>
        ...
        {esAdmin && (
          <div style={{marginTop:10,fontSize:12,color:'#6B0F2B',fontWeight:700,
            background:'rgba(107,15,43,0.06)',padding:'6px 16px',borderRadius:20,
            border:'1px solid rgba(107,15,43,0.15)',display:'inline-block'}}>
            ⚙️ Da clic en el botón de cada persona para editar su jerarquía
          </div>
        )}
      </div>
      ...
      <div style={{display:'flex',flexDirection:'column',alignItems:'center',overflowX:'auto',paddingBottom:20}}>
        <div style={{display:'flex',gap:'clamp(12px,3vw,40px)',justifyContent:'center',alignItems:'flex-start',flexWrap:'wrap'}}>
          {tree.map(node => (
            <NodoOrg key={node.id} node={node} nivel={0}
              esAdmin={esAdmin} empleados={empleados}
              panelId={panelId} setPanelId={setPanelId}
              onActualizar={actualizarEmp}
              onClickEmp={setSel}
            />
          ))}
        </div>
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',alignItems:'center',position:'relative'}}>
      ...
      <div style={{position:'relative'}}>
        <div
          onClick={()=>{ if(!panelAbierto && !editandoCargo) onClickEmp(node); }}
          style={{
            background:'#fff', borderRadius:16,
            padding: nivel===0?'20px 20px 14px':nivel===1?'14px 16px 10px':'10px 12px 8px',
        ...
        >
          {/* Badge departamento nivel 0 */}
          {nivel===0 && (
            <div style={{position:'absolute',top:-10,left:'50%',transform:'translateX(-50%)',
              background:color.bg,color:color.text,fontSize:8,fontWeight:900,
              padding:'2px 10px',borderRadius:20,letterSpacing:1,whiteSpace:'nowrap',zIndex:2}}>
              {(node.departamento||'GENERAL').toUpperCase()}
            </div>
          )}
          ...
          {node.es_asistente && (
            <div style={{position:'absolute',top:-8,right:8,
              background:'#C9A84C',color:'#fff',fontSize:7,fontWeight:900,
              padding:'2px 6px',borderRadius:10,letterSpacing:0.5}}>ASISTENTE</div>
          )}
          ...
          {node.foto_url
            ...
            : <div style={{width:fotoSize,height:fotoSize,borderRadius:'50%',background:`${color.bg}18`,border:`3px solid ${color.bg}`,display:'flex',alignItems:'center',justifyContent:'center',fontSize:nivel===0?24:18,color:color.bg,margin:'0 auto 8px',fontWeight:900}}>
                {node.nombre?.[0]}{node.apellido_paterno?.[0]}
              </div>
          }

          <div style={{fontWeight:800,fontSize:nivel===0?14:nivel===1?12:11,color:'#1a1a2e',lineHeight:1.2}}>
          ...
          {editandoCargo && esAdmin ? (
            <div onClick={e=>e.stopPropagation()} style={{marginTop:6}}>
              <input value={cargoBorrador} onChange={e=>setCargoBorrador(e.target.value)}
                ...
                autoFocus
                onKeyDown={e=>{ if(e.key==='Enter'){ onActualizar(node.id,{cargo_funcional:cargoBorrador}); setEditandoCargo(false); }}}
              />
              <div style={{display:'flex',gap:4,marginTop:4,justifyContent:'center'}}>
                <button onClick={e=>{ e.stopPropagation(); onActualizar(node.id,{cargo_funcional:cargoBorrador}); setEditandoCargo(false); }}
                ...
                <button onClick={e=>{ e.stopPropagation(); setEditandoCargo(false); setCargoBorrador(node.cargo_funcional||node.puesto||''); }}
            ...
            <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:4,marginTop:3}}>
              ...
              {esAdmin && (
                <button onClick={e=>{ e.stopPropagation(); setEditandoCargo(true); }}
                  style={{background:'none',border:'none',cursor:'pointer',fontSize:9,opacity:0.4,padding:0,lineHeight:1}}
                  title="Editar cargo">✏️</button>
              )}
            </div>
          )}

          {/* Botón jerarquía — solo admin */}
          {esAdmin && (
            <button
              onClick={e=>{ e.stopPropagation(); setPanelId(panelAbierto?null:node.id); }}
              style={{
                marginTop:8, padding:'5px 12px', borderRadius:20,
                border:`1px solid ${color.border}`,
                background: panelAbierto ? color.bg : 'transparent',
                color: panelAbierto ? color.text : color.bg,
        ...
        <>
          ...
          {node.hijos.length > 1 && (
            <div style={{height:2,background:'#C9A84C',
              width:`${Math.min(node.hijos.length*(nivel===0?185:155),860)}px`,
              maxWidth:'88vw'}}/>
          )}
          <div style={{display:'flex',gap:nivel===0?'clamp(10px,2.5vw,36px)':'clamp(6px,1.5vw,20px)',
            flexWrap:'wrap',justifyContent:'center',alignItems:'flex-start'}}>
            {node.hijos.map(hijo => (
              <NodoOrg key={hijo.id} node={hijo} nivel={Math.min(nivel+1,2)}
                esAdmin={esAdmin} empleados={empleados}
                panelId={panelId} setPanelId={setPanelId}
                onActualizar={onActualizar} onClickEmp={onClickEmp}
              />
            ))}
          </div>
        </>
...
// ── Panel jerarquía ───────────────────────────────────────
function PanelJerarquia({ emp, empleados, onActualizar, onCerrar, color }) {
  ...
  const guardar = async () => {
    await onActualizar(emp.id, {
      jefe_id: jefeId || null,
      es_asistente: esAsistente,
      departamento: depto || emp.departamento,
  ...
  return (
    <div onClick={e=>e.stopPropagation()} style={{
      ...
      marginTop:6, background:'#fff', borderRadius:14, padding:'16px',
      boxShadow:'0 8px 40px rgba(0,0,0,0.18)', zIndex:100, width:240,
      border:`1.5px solid ${color.border}`,
    }}>
      <div style={{fontWeight:800,fontSize:12,color:color.bg,marginBottom:12,textAlign:'center'}}>
        ⚙️ {emp.nombre} {emp.apellido_paterno}
      </div>

      {/* Reporta a */}
      <label style={{display:'block',fontSize:10,fontWeight:700,color:'#718096',textTransform:'uppercase',letterSpacing:0.5,marginBottom:4}}>
        Reporta a (jefe directo)
      </label>
      <select value={jefeId} onChange={e=>setJefeId(e.target.value)}
        style={{width:'100%',padding:'7px 10px',borderRadius:9,border:`1.5px solid ${color.border}`,
          fontFamily:'Montserrat,sans-serif',fontSize:12,marginBottom:10,boxSizing:'border-box'}}>
      ...
      {depto==='__nuevo__' && (
        <input placeholder="Nombre del departamento" onBlur={e=>setDepto(e.target.value||emp.departamento)}
          style={{width:'100%',padding:'7px 10px',borderRadius:9,border:`1.5px solid ${color.border}`,
            fontFamily:'Montserrat,sans-serif',fontSize:12,marginBottom:10,boxSizing:'border-box'}}/>
      )}
      ...
      <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer',marginBottom:10}}>
        <input type="checkbox" checked={esAsistente} onChange={e=>setEsAsistente(e.target.checked)}
          style={{accentColor:color.bg,width:14,height:14}}/>
        <span style={{fontSize:11,fontWeight:700,color:'#4A5568'}}>Es asistente</span>
      </label>

      {/* Nivel */}
      <label style={{display:'block',fontSize:10,fontWeight:700,color:'#718096',textTransform:'uppercase',letterSpacing:0.5,marginBottom:4}}>
        Prioridad en jerarquía (0 = primero)
      </label>
      <input type="number" min="0" max="99" value={nivel} onChange={e=>setNivel(e.target.value)}
        style={{width:'100%',padding:'7px 10px',borderRadius:9,border:`1.5px solid ${color.border}`,
          fontFamily:'Montserrat,sans-serif',fontSize:12,marginBottom:12,boxSizing:'border-box'}}/>

      <div style={{display:'flex',gap:8}}>
        <button onClick={onCerrar}
          style={{flex:1,padding:'8px',borderRadius:9,border:`1px solid ${color.border}`,
            background:'#fff',cursor:'pointer',fontFamily:'Montserrat,sans-serif',fontWeight:700,fontSize:12,color:'#718096'}}>
          Cancelar
        </button>
